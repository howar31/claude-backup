...
- Acquire the rclone execution lock. If another run already holds it, append `SKIP: already running` to the log and return 0 (see "Execution Lock").
...
- `$LOG_DIR/.rclone.lock` — guards `cmd_drive`.
- `$LOG_DIR/.git-snapshot.lock` — guards `cmd_git_snapshot`.
...
**Concurrency:** Each layer holds an execution lock, so a scheduled tick that overlaps a still-running manual run SKIPs instead of starting a second copy (see "Execution Lock").
